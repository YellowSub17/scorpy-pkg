...
fit, cov_fit =  scipy.optimize.curve_fit(lambda x,a,b,c,d: a/(b*x+c)+d, x,y)
...
plt.plot(xfit, fit[0]/(fit[1]*xfit +fit[2])+fit[3], label='Fit')
plt.xlabel('l')
plt.ylabel('sf')
plt.legend()
...
bl1l_sf = fit[0]/(fit[1]*np.arange(0, bl1.nl) +fit[2]) +fit[3]
