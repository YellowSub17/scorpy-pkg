...
targ_vals, _ = a.get_geometry_vals(sub_tags[0], count='targ', geometry='distances')

vals, errs = [], []

for sub_tag in sub_tags:
    print(sub_tag)
    aval, aerr =  a.get_geometry_vals(sub_tag, geometry='distances')

    vals.append(aval)
    errs.append(aerr)
    a._plot_errorbar(targ_vals, aval, 0, 0,
                    xlabel=f'Target',  ylabel=f'Algo',
                capsize=0, fig=fig, axes=axes, )
...
a._plot_errorbar(targ_vals, algo_vals, 0, 3*total_errs,
                    xlabel=f'Target',  ylabel=f'Algo',
                capsize=2, fig=fig, axes=axes, color=(1,0,0))
