...
for i, (xtal_size, color) in enumerate(zip(xtal_sizes, cmap1)):
    ...
    for exponent in range(nexponents):
        ...
        # np.save(f'{data_dir}/lines/{pdb_code}-{xtal_size}nm-{geom_code}-x0-n{nframes}-css.npy', np.array(css_s))


        css_s = np.load(f'{data_dir}/lines/{pdb_code}-{xtal_size}nm-{geom_code}-x0-n{nframes}-css.npy')
