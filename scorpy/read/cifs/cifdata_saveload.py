...
class CifDataSaveLoad:
    """Mixin handler enabling the exporting of structural data parameters to varying formats."""

    @verbose_dec
    def save(self, path, verbose=0):
        """Write current structural attributes out into standard CIF file block notation.
        ...
        verbose : int, default 0
            Verbosity parameter tracking logging alerts handled by wrapper decorators.
        """
        cif = pycif.CifFile()
    ...
    def save_shelx_hkl(self, path):
        """Export reflections utilizing fixed-width formatting rules standardized by SHELX packages.

        Parameters
        ----------
        path : str
            Destination destination string mapping where files write.
        """
        # I_loc = np.where(self.scat_bragg[:,3]!=0)
        # hklI = self.scat_bragg[I_loc]

        hklI = self.scat_bragg[:]
        hklI[:, -1] = np.abs(hklI[:, -1])
    ...
    def save_crystfel_hkl(self, path):
        """Export properties formatting reflection components using CrystFEL syntax conventions.

        Parameters
        ----------
        path : str
            Destination filepath indicator string.
        """
        f = open(path, "w")
