...
class CifDataFill:
    """A mixin class to populate, fill, and calculate scattering data in different coordinate systems."""

    def make_support(self):
        """Set the intensity/weight of the last column to 1 across all scattering representations."""
        self._scat_bragg[:, -1] = 1
        self._scat_rect[:, -1] = 1
        self._scat_sph[:, -1] = 1

    def calc_scat(self, h, k, l, I, fill_missing=True):
        """Calculate and update reciprocal grid locations in Bragg, rectilinear, and spherical coordinates.

        Parameters
        ----------
        h : array_like
            Miller index h components.
        k : array_like
            Miller index k components.
        l : array_like
            Miller index l components.
        I : array_like
            Measured diffraction intensities corresponding to the hkl indices.
        fill_missing : bool, default True
            If True, missing reflections within the bounding grid or resolution limit are filled with NaN.
        """
    ...
    def make_qmax(self):
        """Determine and enforce the maximum reciprocal space distance restriction (qmax)."""
    ...
    def fill_missing(self, hklI, ab_flag="b"):
        """Identify missing reflection coordinates in the lattice space and insert them with NaN intensities.

        Parameters
        ----------
        hklI : ndarray
            An (N, 4) array where columns represent h, k, l, and intensity.
        ab_flag : {'a', 'b'}, default 'b'
            Flag indicating the filling logic boundary.
            'b' uses a box bounded by max detected absolute h, k, l.
            'a' expands bounds radially out to the resolution limit (`qmax`).

        Returns
        -------
        ndarray
            The augmented array containing both original and missing reflection targets.
        """
        if ab_flag == "b":
    ...
    def fill_from_cifdict(self, cif_dict, sep, fill_missing=False):
        """Extract scattering elements from a CIF-derived dictionary and process them into coordinates.

        Parameters
        ----------
        cif_dict : dict
            Dictionary parsed from a Crystallographic Information File (.cif).
        sep : str
            Separator syntax used in keys (typically '_' or '.').
        fill_missing : bool, default False
            Whether to insert missing reflections with NaN.
        """
        h = (
            np.array(cif_dict[f"_refln{sep}index_h"])
            .astype(float)
            .astype(np.int32)
        )
        k = (
            np.array(cif_dict[f"_refln{sep}index_k"])
            .astype(float)
            .astype(np.int32)
        )
        l = (
            np.array(cif_dict[f"_refln{sep}index_l"])
            .astype(float)
            .astype(np.int32)
        )

        inten_pow_dict = {
            f"_refln{sep}intensity_meas": 1,
            f"_refln{sep}f_squared_meas": 1,
            f"_refln{sep}f_meas_au": 2,
        }

        inten_key = list(
            set(inten_pow_dict.keys()).intersection(cif_dict.keys())
        )[0]
    ...
    def fill_from_vhkl(self, path, fill_missing=False):
        """Populate scattering data from a `.vhkl` file format.

        Parameters
        ----------
        path : str
            Filepath pointing to the vhkl file source.
        fill_missing : bool, default False
            Whether to fill missing reflections with NaN.
        """
    ...
    def fill_from_shelx_hkl(self, path, fill_missing=False):
        """Parse structured Fortran format outputs typically generated by SHELX (.hkl).

        Parameters
        ----------
        path : str
            Path to the SHELX input reflection file.
        fill_missing : bool, default False
            Whether to systematically populate omissions inside bounds with NaN.
        """
    ...
    def fill_from_sphv(self, sphv, fill_missing=False):
        """Generate crystal metrics mapped via continuous spherical volume indices.

        Parameters
        ----------
        sphv : Object
            Spherical volume data representation tracking grid indexes and field limits.
        fill_missing : bool, default False
            Whether to pad missing items inside bounds with NaN.
        """
        ...
        for i, (q_ind, theta_ind, phi_ind) in enumerate(
            zip(q_inds, theta_inds, phi_inds)
        ):
